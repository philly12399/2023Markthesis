...
if [ -n "$1" ] ; then
    ...
    poetry run main --config '${module_config}/pcd-detect-centerpoint-lidar1.json5' \
    --pcd_files_dir /mnt/nfs/wayside_team/mark_thesis_data/self-training-data/pcd-files/pointcloud-lidar1 \
    --num_frames $1"
else
    start sh -c "cd '${repo_dir}/py-bin/pcd-detect' && \
    poetry run main --config '${module_config}/pcd-detect-centerpoint-lidar1.json5' \
